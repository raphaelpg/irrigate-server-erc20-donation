...
describe("test web3 functions", () => {
  ...
  test("should return address balance", async () => {
    const initialBalance = await web3Functions.getERC20Balance(config.web3.owner);
    
    const web3 = new Web3(config.web3.localProvider);
    const erc20Address = config.web3.erc20;
    const erc20Instance = new web3.eth.Contract(erc20Interface.abi as any, erc20Address);
    const amount = 1000;
    await erc20Instance.methods.mint(config.web3.owner, amount).send({ from: config.web3.owner });
    
    const finalBalance = await web3Functions.getERC20Balance(config.web3.owner);
    
    expect(parseInt(finalBalance)).toEqual(parseInt(initialBalance) + amount);
  });
});